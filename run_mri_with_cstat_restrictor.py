...
from sixsa_utils import compute_cstat , generate_function_for_cstat_restrictor , \
    compute_x_sim , print_message , print_best_fit_parameters , goodbye_message , welcome_message , \
    generate_function_for_cstat_restrictor
...
if __name__ == '__main__' :
    ...
    with open(path_yml_file+'mri_config_cstat_restrictor.yml' , 'r') as config_file :
    ...
    num_rounds_for_cstat_restrictor = config['num_rounds_for_cstat_restrictor']
    num_sim_for_cstat_restrictor = config['num_sim_for_cstat_restrictor']
    ...
    generate_restrictor_function_kwargs = {"x_obs" : x_obs ,
                                           "good_fraction_in_percent" : 25.0}
    ...
    cstat_restrictor = [prior]
    duration_cstat_restrictor = 0.
    for r in range(num_rounds_for_cstat_restrictor) :
        print(f"Doing round {r + 1:d}")
        start_time = time.perf_counter( )
        theta_int = cstat_restrictor[-1].sample((num_sim_for_cstat_restrictor ,))
        ...
        print(f'It took {end_time - start_time: 0.2f} second(s) to complete '
              f'{num_sim_for_cstat_restrictor:d} simulations at round {r + 1:d}')

        start_time = time.perf_counter( )
        restriction_estimator.append_simulations(theta_int , torch.as_tensor(x_int))

        if r < num_rounds_for_cstat_restrictor - 1 :
        ...
        cstat_restrictor.append(restricted_prior)
        end_time = time.perf_counter( )
        print(f'It took {end_time - start_time: 0.2f} second(s) to run the classifier on '
              f'{num_sim_for_cstat_restrictor:d} simulations at round {r + 1:d}')

    end_time = time.perf_counter( )
    duration_cstat_restrictor += end_time - start_time_to_get_the_restricted_prior
    print(f"The whole process took {duration_cstat_restrictor:.1f} seconds.")
    ...
    theta_from_restricted_prior = cstat_restrictor[-1].sample((10 * n_posterior_samples ,))
    ...
    c.add_chain(Chain(samples = df_theta_from_restricted_prior ,
                      name = f"Restricted prior ({num_sim_for_cstat_restrictor:d} x {num_rounds_for_cstat_restrictor:d})" ,
    ...
    #
    theta_train = cstat_restrictor[-1].sample((number_of_simulations_for_train_set ,))
    ...
    table_data = [
        ("Restrictor " , f"{duration_cstat_restrictor:.2f}") ,
